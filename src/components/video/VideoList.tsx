...
const VideoList: React.FC<VideoListProps> = ({ videos, onSelect, onDownloadSelect, favorites = [], onToggleFavorite, onDeleteHistory }) => {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6 w-full">
      {videos.map((video) => (
        <div key={video.id} className="relative group/list">
          <VideoCard
            title={video.name}
            poster={video.pic}
            type={video.type}
          ...
          {onDeleteHistory && (
            <button
              ...
              className="absolute -top-2 -right-2 bg-red-500 text-white p-1 rounded-full shadow-lg opacity-0 group-hover/list:opacity-100 transition-opacity z-20"
              title="删除记录"
            >
              <X size={14} />
            </button>
